...
import pepper
from pepper.exceptions import PepperArgumentsException, PepperAuthException, PepperException
...
class PepperCli(object):
    ...
    def get_parser(self):
        return optparse.OptionParser(description=__doc__, usage="%prog [opts]", version=pepper.__version__)
    ...
    def parse(self):
        ...
        self.parser.add_option(
            ...
            default=os.environ.get(
                "MASTER_CONFIG", os.path.join(os.path.expanduser("~"), ".config", "pepper", "master")
            ),
            help=textwrap.dedent(
                """
    ...
    def add_authopts(self):
        ...
        optgroup.add_option(
            "-u", "--saltapi-url", dest="saltapiurl", help="Specify the host url.  Defaults to https://localhost:8080"
        )
        ...
        optgroup.add_option(
            "-x",
            dest="cache",
            default=os.environ.get("PEPPERCACHE", os.path.join(os.path.expanduser("~"), ".peppercache")),
            help=textwrap.dedent(
                """
    ...
    def add_retcodeopts(self):
        """Add ret code validation options."""
        optgroup = optparse.OptionGroup(
            self.parser, "retcode Field Validation Options", "Validate return.HOST.retcode fields"
        )

        optgroup.add_option(
            "--fail-any", dest="fail_any", action="store_true", help="Fail if any of retcode field is non zero."
        )
        ...
        optgroup.add_option(
            "--fail-all", dest="fail_all", action="store_true", help="Fail if all retcode fields are non zero."
        )
    ...
    def get_login_details(self):
        """
        Parse config file, environment variables and command line options and returns the config values.
    ...
    def poll_for_returns(self, api, load):
        """Run a command with the local_async client and periodically poll the job cache for returns for the job."""
