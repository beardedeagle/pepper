...
from urllib.request import HTTPHandler, HTTPSHandler, Request, build_opener, install_opener, urlopen
...
class Pepper(object):
    """
    A thin wrapper for making HTTP calls to the salt-api rest_cherrpy REST interface.
    ...
    def __init__(self, api_url="https://localhost:8000", debug_http=False, ignore_ssl_errors=False):
        ...
        if split.scheme not in ["http", "https"]:
            raise PepperException("salt-api URL missing HTTP(s) protocol: {0}".format(api_url))
    ...
    def req_stream(self, path):
        """
        Thin wrapper to get a response from saltstack api.
    ...
    def req_get(self, path):
        """
        Thin wrapper from get http method of saltstack api.
    ...
    def req(self, path, data=None):
        """
        Thin wrapper around urllib2 to send requests and return the response.
    ...
    def req_requests(self, path, data=None):
        """
        Thin wrapper around request and request_kerberos to send requests and return the response.
    ...
    def login(self, username=None, password=None, eauth=None, **kwargs):
        """Authenticate with salt-api and return the user permissions and authentication token or an empty dict."""
        local = locals()
        kwargs.update(
            dict((key, local[key]) for key in ("username", "password", "eauth") if local.get(key, None) is not None)
        )
